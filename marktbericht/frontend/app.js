...
fetch(API + '/health').then(r => r.json()).then(h => {
  $('healthPill').textContent =
    `DB ${h.db ? 'OK' : 'aus'} · Geoapify ${h.geoapify ? 'an' : 'aus'} · KI ${h.ai_mode}`;
  renderChips(h);
}).catch(() => { $('healthPill').textContent = 'Backend nicht erreichbar'; });

function renderChips(h) {
  const chips = [
    ['Geoapify', h.geoapify], ['OpenStreetMap', true],
    ['GeoMap', h.geomap], ['Destatis', h.destatis],
    ['BORIS', h.boris], ['OpenAI', h.ai_enabled],
  ];
  $('srcChips').innerHTML = chips.map(([n, on]) =>
    `<span class="chip ${on ? 'on' : ''}">${on ? '●' : '○'} ${n}</span>`).join('');
}
...
async function generate() {
  ...
  const pushStep = (msg) => {
    ...
    prog.innerHTML = steps.slice(-6).map((s, i, arr) => {
  ...
  if (prog) { prog.classList.remove('hide'); prog.innerHTML = ''; }
...
function render(out) {
  ...
  // DealPilot-Marktbewertung-Karte (kompakte Tachos, Spanne-Umschalter) ganz oben
  try {
    const host = $('mbCard');
    if (host && window.DealPilotMarktbewertung) {
      const mvv = (d.valuation && d.valuation.market_value) || {};
      const rentS = d.rent || {};
      const areaC = (d.ref && d.ref.living_area) || null;
      const rl = (areaC && rentS.q25_per_sqm) ? Math.round(rentS.q25_per_sqm * areaC) : null;
      const rm = (areaC && rentS.median_per_sqm) ? Math.round(rentS.median_per_sqm * areaC) : null;
      const rh = (areaC && rentS.q75_per_sqm) ? Math.round(rentS.q75_per_sqm * areaC) : null;
      if (mvv.estimated != null && mvv.low != null && mvv.high != null && rl != null && rm != null && rh != null) {
        const fmtSq = (v, dec) => v != null ? v.toLocaleString('de-DE', { maximumFractionDigits: dec }) : '–';
        const cardData = {
          rating: (d.deal_score && d.deal_score.rating) || 'Marktwert',
          confidence: { label: mvv.confidence_label || '–', pct: mvv.confidence_pct != null ? mvv.confidence_pct : '–' },
          meta: {
            mikro: (d.micro && d.micro.score != null) ? d.micro.score + ' · ' + _scoreWord(d.micro.score) : '–',
            makro: (d.macro && d.macro.score != null) ? d.macro.score + ' · ' + _scoreWord(d.macro.score) : '–',
            trend: d.price_trend_pct != null ? (d.price_trend_pct > 0 ? '+' : '') + fmtSq(d.price_trend_pct, 1) + '%/J' : '–',
          },
          marktwert: { low: mvv.low, med: mvv.estimated, high: mvv.high,
            sqm: mvv.basis_median_sqm ? Math.round(mvv.basis_median_sqm).toLocaleString('de-DE') + ' €/m²' : '' },
          miete: { low: rl, med: rm, high: rh,
            sqm: rentS.median_per_sqm ? fmtSq(rentS.median_per_sqm, 2) + ' €/m² kalt' : '' },
        };
        if (window._mbCardInst) window._mbCardInst.update(cardData);
        else window._mbCardInst = DealPilotMarktbewertung.mount('#mbCard', cardData);
        host.style.display = '';
      } else { host.style.display = 'none'; }
    }
  } catch (e) { console.warn('Marktbewertung-Karte:', e); }
  ...
  if (cn) {
    ...
    if (ge != null) {
      cn.textContent = ge === 0
        ? 'GeoMap-Kosten dieses Berichts: 0,00 € (aus Cache – keine neuen Abrufe)'
        : 'GeoMap-Kosten dieses Berichts: ' + ge.toFixed(2).replace('.', ',') + ' € netto'
