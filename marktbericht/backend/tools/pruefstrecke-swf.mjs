// pruefstrecke-swf.mjs   (v1085-WPRF)
//
...
function pruefeBeleg(s) {
  ...
  if (b.art === 'zaehlpruefung') {
    ...
    if (!z) {
      ...
      if (s.formel.stufen) {
        ...
        if (soll != null && n !== soll) {
          return fehl(s, `${n} Stuetzstellen statt ${soll}`);
        }
        /* Eckwerte aus dem Dokument, wenn der Beleg sie nennt. */
        for (const e of (b.soll_eckwerte || [])) {
          ...
          if (!r.verfuegbar || !trifft(ist, soll_w, stellenVon(soll_w))) {
            return fehl(s, `Eckwert ${JSON.stringify(ein)}: ${ist} statt ${soll_w}`);
          }
        }
        return gut(s, `Zaehlpruefung ${n} Stuetzstellen`
          + (b.soll_eckwerte ? ` · ${b.soll_eckwerte.length} Eckwerte getroffen` : ''));
      }
  ...
  if (b.art === 'formel_gegen_tabelle') {
    ...
    const z = s.formel.zellen || {};
    const zeilen = Object.keys(z).length;
    const breite = new Set(Object.values(z).map((r) => r.length));
    if (breite.size !== 1) return fehl(s, 'Zeilen unterschiedlich lang');
    const sp = [...breite][0];
    if (b.soll_zellen != null && zeilen * sp !== b.soll_zellen) {
      return fehl(s, `${zeilen * sp} Zellen statt ${b.soll_zellen}`);
    }
    for (const e of (b.soll_eckwerte || [])) {
      const { wert: soll_w, ...ein } = e;
      const r = auswerten(modellAus(s), ein);
      const ist = r.dokumentwert != null ? r.dokumentwert : r.wert;
      if (!r.verfuegbar || !trifft(ist, soll_w, stellenVon(soll_w))) {
        return fehl(s, `Eckwert ${JSON.stringify(ein)}: `
          + `${r.verfuegbar ? ist : r.grund} statt ${soll_w}`);
      }
    }
    return gut(s, `Formel gegen Tabelle: ${zeilen} x ${sp} Zellen · `
      + `${(b.soll_eckwerte || []).length} Eckwerte`);
  }
