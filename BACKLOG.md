...
| B2 | Sensible Rechenlogik ins Backend verschieben, soweit technisch möglich | **BEGONNEN v1360 — der Analyse-Prompt ist raus.** 78 Zeilen aus `ui.js`: Bewertungsskala mit vorgeschriebenem Wortlaut, Anti-Halluzinations-Regeln, Ausgabestruktur. **Die Verlagerung war schon passiert, nur das Aufräumen fehlte** — `openaiService.js` baut denselben Prompt seit Langem serverseitig, und `/ai/status` meldet `available:true` hart verdrahtet, womit der Frontend-Prompt seit V26 unerreichbar war. Mit ihm fiel `_runAIClient()`, das `api.openai.com` direkt aus dem Browser rief; die Funktion bleibt über `/ai/analyze` mit `userApiKey`. **167 Zeilen weniger im ausgelieferten Code.** Als Nächstes: der Deal Score (`dealscore2.js`) — hoher Wert, aber die App rechnet live bei jeder Eingabe, also vorher eine Demo für Marcel. |
