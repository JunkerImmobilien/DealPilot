...
| A8 | **Drei Felder ohne Leser** — ENTSCHEIDUNG OFFEN | **GEMESSEN v1355:** `min_dscr`, `min_cashflow_vor_st` und `max_ltv` („Persönliche Mindest-Schwellen" unter Standardwerte) werden vom Formular geschrieben, in `config.js` vorbelegt (1,20 · 0 € · 90 %) — und **von niemandem gelesen.** Gegengeprüft über alle Profil-Konsumenten: `main.js` fragt genau sieben Schlüssel ab, keiner davon ist dabei; im Backend und in jeder Anzeige kein Treffer. Damit hat sich die Frage nach zwei konkurrierenden Schwellensystemen erledigt — es gibt nur eines. **Zwei Wege: anschließen** (als persönliche Ampel neben den Score: „dein Mindest-DSCR ist 1,20, dieses Objekt liegt bei 1,08") **oder wegräumen.** Marcel entscheidet. |
