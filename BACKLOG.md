...
| A5 | **Datenraum** und **externe Anbieter**: passt das noch? | **GEPRÜFT.** Datenraum inhaltlich vollständig, aber `dp_datenraum_v141` liegt **nur im localStorage** — Gerätewechsel kostet alle Verknüpfungen, und aus diesem Bereich gehen Bank-Anfragen raus. Gehört zu Block B. Externe Anbieter: zwei Regelverstöße behoben (v1354) — `ae/oe/ue` im Nutztext und die namentliche Nennung von PriceHubble. Der Reiter selbst trägt **einen** Anbieter; Zusammenlegung steht unter A6 zur Entscheidung. |
...
| A9 | **Anbieter-Neutralität** — ENTSCHEIDUNG OFFEN, GRÖSSER ALS GEDACHT | Zuerst in der PRE-FLIGHT-Karte gefunden (Logos mit `alt="Sprengnetter"` / `alt="PriceHubble"`). Beim Prüfen des Quick-Check-Prompts zeigte sich: das ist kein Einzelfall. **Sechs Orte:** PRE-FLIGHT-Logos · Quick-Check-Auswahlkacheln mit Logos und `title` · die Meldung „PriceHubble wird angefragt…" · `provider:`/`source:` in den Daten · der KI-Prompt Z. 5309 („AVM-Daten (PriceHubble)") · Einstellungen (**behoben v1354**). CLAUDE.md: beide werden nach außen nie namentlich genannt, ImmoMetrica darf. **Nichts angefasst** — es ist ein Thema, nicht sechs Fundstellen, und gehört als ein Vorgang entschieden. Ein Flickenteppich (Prompt geändert, Logos stehen gelassen) wäre schlechter als beides. |
