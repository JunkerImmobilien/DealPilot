...
| B21 | **Direktweg zu OpenAI aus dem Browser** | **HALB ERLEDIGT v1360.** Der Weg in `ui.js` ist weg. Der zweite, in `quickcheck-app.html` Z. 5290 ff. (`runRealAi`), steht noch: **gemessen hat der Quick-Check keine Auth-Anbindung** — `Auth.apiCall` kommt dort null mal vor, er spricht mit der Hauptanwendung nur über `postMessage` (qc-bridge). Der Umbau heißt: Anfrage über die Brücke reichen, authentifiziert weiterleiten, Antwort zurückspielen, plus ein Backend-Endpunkt, weil das Payload-Format auf keinen bestehenden passt. **Eigenes Paket, kein Nebenschritt.** |
