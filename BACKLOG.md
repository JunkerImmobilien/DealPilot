...
| B2 | Sensible Rechenlogik ins Backend verschieben, soweit technisch möglich | **LÄUFT — v1360 bis v1363.** Marcels Vorgabe „keine Einschränkungen" schließt die Serververlagerung des Scores vorerst aus (Latenz bei jeder Eingabe), also zuerst alles, was Schutz bringt, ohne etwas zu verändern. **v1360:** der Analyse-Prompt raus (167 Zeilen), Direktweg zu OpenAI zu. **v1361:** zweiter § 32a-Tarif in `dashboard.js` entfernt — er war bereits auseinandergelaufen (Grundfreibetrag 11.784 statt 11.604, `round` statt `floor`, bis 43 € Abweichung). **v1362/v1363:** eine Eingabe löste **acht** Score-Berechnungen aus, jetzt **zwei** — Ursache war ein `updHeaderBadges()` vor dem Cache und ein zweiter Cache-Name, der eigenständig entstand. **Das ist die Voraussetzung für jede Verlagerung:** acht Anfragen je Eingabe wären am Limit von 100/Minute gescheitert, eine ist machbar — die Serverrunde dauert **34 ms**. |
