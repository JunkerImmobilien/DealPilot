...
| 6 | Ist-/Soll-Modell + erweiterter MFH-Konfigurator (Einheiten) | Konzept | **STUFE 1 ERLEDIGT v1448** (`e7d287c`) — Marcel: „was würdest du machen?" → Einheitenliste für MFH/ZFH/Gewerbe: Einstieg unter der Wohnfläche, Nr./Lage/Art/Fläche/Zimmer/Ist-/Soll-Miete/Status/Zustand, Zeilen duplizieren; Übernehmen schreibt Fläche, Wohneinheiten, Ist-Kaltmiete (vermietete) — calc unverändert; gespeichert als `_mfh`. Im Browser: 3 Einheiten → 226 m², 2 WE, 1.380 € Ist, Leerstand 1 WE. **Nächste Stufen** (Soll in Kennzahlen, Maßnahmenplan, Vererbung) im Konzept. |
...
| 21 | Light Mode Redesign für Steuerkanzleien — 5–6 Layout-Konzepte, **nichts implementieren** vor Auswahl | Konzept | **DEMO STEHT** — richtige klickbare Demo aller sechs Layouts mit echten Beispielobjekten, Objektwechsel, Reitern, Sortierung, einklappbarer Leiste, Umschalter Desktop/Tablet/Handy: https://claude.ai/artifact/5vWnSbZQsogMGfweKfJnRY (Kopie `design/Vorschläge/light-mode-kanzlei-demo.html`). **Wartet auf Marcels Auswahl** — dann wird gebaut. |
| 22 | Steuern: AfA manuell, RND ermitteln/übernehmen über den **vorhandenen** Kern, Vererbung Objekt → Gebäude → Einheit, RND-Gutachten auslösen | Konzept | **ERLEDIGT v1447 (Auftragsweg)** (`d17e85e`) + v1439 — Marcel: „direkt an gutachten.org". Der RND-Wizard öffnet ein Bestätigungsfenster mit allen Daten, Einverständnis-Haken, dann E-Mail an support@gutachten.org (eigenes Mailprogramm) oder deren kostenlose Ersteinschätzung; DealPilot sendet nichts selbst an Dritte; Status „angefragt" am Objekt. Partnerkarte Gutachten.org hat auf Staging die Ziel-Mail. **Offen aus dem Konzept:** AfA „eigener Satz", RND-Karte im Steuer-Tab, Vererbung. |
