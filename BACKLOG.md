...
| B22 | **Drei Grundfreibeträge im Code** — FACHFRAGE FÜR MARCEL | Beim Zusammenführen der Steuertarife (v1361) gemessen: `tax.js` rechnet mit **11.604**, `dashboard.js` rechnete mit **11.784** (jetzt zusammengeführt), und `rnd-calc.js:492` schätzt den Grenzsteuersatz mit den Zonengrenzen **12.096** / 17.443 / 68.480. Drei verschiedene Werte — und `tax.js` trägt dabei den Kommentar „Tarif 2026". Welcher Jahrgang gelten soll, ist eine Bewertungsfrage und gehört Marcel. Die Struktur ist jetzt vorbereitet: wer den Tarif ändert, ändert ihn in `tax.js` und es wirkt überall. |
