...
| 5 | MFH + hohe Sanierung: LTV > 400 % — Nenner prüfen, Kaufpreis / Ist-Wert / Wert nach Sanierung trennen | Analyse → Vorschlag | **VORSCHLAG STEHT** — `design/Vorschläge/ltv-sanierung-value-add.md`. Ursache: Nenner ist Kaufpreis (bzw. Wert heute), das Darlehen finanziert aber auch die Sanierung; EK-Anzeige `100 − LTV` wird negativ. Vorschlag: Wert nach Sanierung als Feld, LTC + LTV nach Sanierung bei Sanierungsfinanzierung, EK-Balken auf GI, BMY zusätzlich auf GI. Nebenbefund: `calc.js` rechnet alle Kennzahlen neben `DealKpis` her (Doppelung → Punkt 19). **Wartet auf Marcel: Schwelle, Score-Kennzahl, Soll-Wert ohne Gutachten.** |
| 6 | Ist-/Soll-Modell + erweiterter MFH-Konfigurator (Einheiten) | Konzept | **KONZEPT STEHT** — `design/Vorschläge/mfh-ist-soll-konfigurator.md`: UX (optionaler Einstieg, Modal, Duplizieren), Datenmodell Objekt → Gebäude → Einheit mit Vererbung, Aggregation in die bestehenden Felder (calc rechnet unverändert), Kennzahlen Ist gegen Soll, vier Umsetzungsstufen. **Wartet auf Marcel** (ein Gebäude? CSV? Summen sperren? Soll-Miete Sprung oder Staffel?). |
...
| 8 | Bruttofläche (BGF DIN 277) automatisch ableiten, typabhängig, überschreibbar, Annahme zeigen | Bau | **TEIL 1 ERLEDIGT v1434** — die Ableitung gab es schon im Rechenkern (`nhk2010.js` bgf(): direkt · Haus × 1,55 als Näherung · MFH keine · ETW nur direkt), im Bericht mit Hinweis. Neu: unter dem Feld im Objekt-Tab steht, was der Rechenkern tun wird, mit Zahl. **Teil 2 wartet auf Marcel:** typabhängige Faktoren (Keller, Dachausbau, MFH) — jeder Faktor braucht eine **Quelle** (Doktrin: keine erfundene Zahl). Welche Literatur/welcher Ausschuss soll gelten? |
...
| 12 | Deal-Aktionen „Ready für die Bank" heller und seriöser | Design | **DEMO STEHT** — `design/Vorschläge/bank-und-score-hell-demo.html` (A weiß-ruhig passend zur Bankfassung v1436, B Creme mit Bankblau). Wartet auf Marcels Wahl; Umsetzung **BLOCKIERT:** braucht Messungen im Browser (Kaskade, Handy-Breite, Mikrofon) — die Chrome-Erweiterung war am 18.09. nicht verbunden. |
| 13 | Deal-Score + Investor-Score hell/dunkel, folgt der globalen Einstellung | Design | **DEMO STEHT** — gleiche Datei: Creme (Hero-Palette umgekehrt) oder Weiß; Scores folgen dem globalen Profil, optional „Scores immer dunkel". Wartet auf Marcels Wahl; Umsetzung **BLOCKIERT:** braucht Messungen im Browser (Kaskade, Handy-Breite, Mikrofon) — die Chrome-Erweiterung war am 18.09. nicht verbunden. |
...
| 17 | Objektzustand: Sterne → verbale Skala, ggf. Komponenten; API-Mapping, Altbestand lesbar | Analyse → Vorschlag | **VORSCHLAG STEHT** — `design/Vorschläge/zustand-und-schnittstellen.md`. Befund: sechs Skalen für einen Begriff; **die 8 Sterne gehen an keine Bewertungsschnittstelle**; die DealPilot-Karte rechnet jedes Objekt als „gepflegt". Vorschlag: eine verbale 5-Stufen-Skala je Komponente + Gesamtstufe, feste Übersetzung je Anbieter, Altbestand 1:1 lesbar. **Wartet auf Marcel** (Skala, 0,82 statt 0,88, Karte mitschicken). |
...
| 21 | Light Mode Redesign für Steuerkanzleien — 5–6 Layout-Konzepte, **nichts implementieren** vor Auswahl | Konzept | **KONZEPTE STEHEN** — `design/Vorschläge/light-mode-kanzlei-konzepte.html`: Analyse des heutigen hellen Modus (Bezug `design/mockups/hell.png`), sechs Layouts mit Drahtmodell, Vor-/Nachteilen, Vergleichstabelle und Kombinationsempfehlung (Basis Workspace + einklappbare Leiste + Portfolio-Tabelle als Start). **Wartet auf Marcels Auswahl.** |
| 22 | Steuern: AfA manuell, RND ermitteln/übernehmen über den **vorhandenen** Kern, Vererbung Objekt → Gebäude → Einheit, RND-Gutachten auslösen | Konzept | **KONZEPT STEHT** — `design/Vorschläge/afa-rnd-gutachten.md`. Befund: AfA nur feste Auswahl; **drei** RND-Rechnungen; Wizard fragt `mod_punkte` neu ab; Ergebnis nie im Steuer-Tab; drei Anfragewege ohne Status, Gutachten.org-Karte ohne Ziel-Mail. Zwei Fehler behoben (v1439: AfA-Hinweis ohne Baujahr, Wizard GND 70 statt 80). **Wartet auf Marcel** (Auftragsziel, Schwelle, indikative RND als AfA?, Status-Pflege). Deckt R3–R5 mit ab. |
