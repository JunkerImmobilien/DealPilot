...
// V272.1b-rechtl-cleanup: Consent-Status-Helper (synchron, kein Observer)
function _dpGetConsentStatusHTML() {
  try {
    var stored = localStorage.getItem('dp_legal_accepted');
    if (!stored) {
      return 'Du bist Bestandsnutzer — die rechtlichen Hinweise wurden als akzeptiert markiert (Legacy).';
    }
    var data = JSON.parse(stored);
    if (data.version === 'legacy-v0') {
      return '<strong>Bestandsnutzer:</strong> Als Legacy akzeptiert markiert (kein expliziter Consent erforderlich).';
    }
    if (data.accepted_at) {
      var dt = new Date(data.accepted_at).toLocaleString('de-DE');
      return '<strong>Akzeptiert:</strong> Version ' + data.version + ' am ' + dt;
    }
    return 'Status: ' + (data.version || 'unbekannt');
  } catch(e) {
   ...
   +
    ...
    '<div class="info-section" id="info-section-hinweise" style="margin-top:24px">' +
      ...
      '<div class="info-legal-text">' +
        ...
        '<p>KI-Inhalte werden über OpenAI generiert und können Fehler enthalten — jede KI-Aussage durch qualifizierten Berater verifizieren lassen.</p>' +
        '<p><a href="#" onclick="if(window.DealPilotLegal){DealPilotLegal.showInfo();return false;}" style="color:#C9A84C;text-decoration:underline">Vollständige Hinweise mit 12 Sections anzeigen (Version 1.1) →</a></p>' +
      '</div>' +
    '</div>' +
...
/* === v646-logo-rahmen ==============================================
   ...
   =================================================================== */
(function(){
  var DARK  = 'assets/dealpilot-logo-rahmen.png';       /* weisse Wortmarke -> dunkler Grund */
  var LIGHT = 'assets/dealpilot-logo-rahmen-hell.png';  /* dunkle Wortmarke -> heller Grund  */
  ...
  function sync(){
    ...
    try{
      ...
      if (src.indexOf('dealpilot-logo-rahmen') < 0) return;
