...
(function () {
  ...
  function toast(m) {
    try {
      ...
      t.textContent = m;
      t.style.cssText = 'position:fixed;left:50%;bottom:26px;transform:translateX(-50%);z-index:100001;background:#0b0b0a;color:#fff;border:1px solid #C9A84C;border-radius:10px;padding:12px 20px;font:600 13.5px Inter;box-shadow:0 12px 40px rgba(0,0,0,.5);pointer-events:none';
  ...
  function injectCss() {
    ...
    s.textContent = [
      ...
      '.mf-modal{width:100%;max-width:720px;max-height:92vh;background:#fff;border-radius:16px;overflow:hidden;border:1px solid rgba(201,168,76,.4);box-shadow:0 40px 100px rgba(0,0,0,.6);display:flex;flex-direction:column}',
      '.mf-bar{background:#0b0b0a;display:flex;align-items:center;padding:14px 22px}',
      '.mf-logo{font:700 18px "Space Grotesk";color:#fff}.mf-logo b{color:#C9A84C}',
      '.mf-ctx{margin-left:auto;font:600 11px "JetBrains Mono";letter-spacing:.22em;color:#C9A84C;text-transform:uppercase}',
      ...
      '.mf-hero{background:linear-gradient(105deg,#E8CC7A,#C9A84C 52%,#b8932f);padding:20px 26px}',
      ...
      '.mf-label{font:600 10px "JetBrains Mono";letter-spacing:.15em;text-transform:uppercase;color:#9a7f3a;margin:0 0 12px}',
      ...
      '.mf-th{width:40px;height:40px;border-radius:9px;background:#f6ecd0;color:#b8932f;display:flex;align-items:center;justify-content:center;font-size:17px;flex-shrink:0}',
      ...
      '.mf-btn{font:600 12px Inter;padding:7px 12px;border-radius:8px;border:1px solid #e8d9a8;background:#fff;color:#b8932f;cursor:pointer;white-space:nowrap}.mf-btn:hover{background:#faf4e6}',
      ...
      '.mf-bg.ein{background:#fbf1dc;color:#b8932f}.mf-bg.best{background:#e2f0e6;color:#3FA56C}.mf-bg.zur{background:#fbeeec;color:#B86250}',
  ...
  async function load() {
    ...
    try {
      var or = await api('/objects?limit=100'); var objs = (or && or.objects) || [];
      ...
      }).join('') : '<p class="mf-ph">Alle Objekte sind bereits freigegeben.</p>';
