'use strict';
/* v405-bridge-backing ============================================================
   ROLLE DIESER DATEI (Stand QC-Integration Etappe B, v405):
   Diese Datei ist BRIDGE-BACKING-LOGIK fuer das iframe-QuickCheck
   (frontend/quickcheck-app.html / QcApp). Das iframe ist die AKTIVE QC-UI.
   Die fruehere Parent-QC-Oberflaeche ist seit v400 stillgelegt (Render-Block raus).

   OEFFENTLICH GENUTZTE API (NICHT entfernen - hat externe Aufrufer):
     - showQuickCheck(), closeQuickCheck(), qcCalc()   <- ui.js (enterQuickCheckMode)
     - qcSaveAsObject()                                 <- qc-bridge.js (_handleSave),
                                                           auto-save.js (Wrapper),
                                                           index.html (v236 Tab-Switch-Wrapper)
     - qcImportPdfTrigger()                             <- qc-bridge.js (_handleImportPdf)
   Die Bruecke (qc-bridge.js) kennt nur 2 iframe-Nachrichten: qc-save und qc-import-pdf.

   HINWEIS: Weitere qc*-Exporte (qcSetLtv, qcEkChanged, qcBewirtMode/Period, qcAiFillAll,
   qcHandlePhotos/qcDelPhoto, qcAiResearchInfo/qcCloseAiInfo/qcApplyAiMiete,
   qcImportFromUrl/qcRunUrlImport/qcCancelUrlImport, qcMieteAddUp, _qcSyncFromMain)
   stammen aus der alten Parent-UI und sind aktuell ohne externen Aufrufer. Sie werden
   beim Laden NICHT ausgefuehrt (nur definiert) und sind daher unkritisch. Entfernen ist
   ein optionaler spaeterer Aufraeumschritt (Etappe B2) - nur nach Einzel-Grep je Funktion.
   ============================================================================= */
/* ═══════════════════════════════════════════════════════════════════════════
...
(function() {
  ...
  function showQuickCheck() {
    /* v400-slim-showqc: Der QuickCheck wird ueber die iframe-Seite (quickcheck-app.html)
       gerendert. Das frueher hier eingebettete Parent-Markup (alter QC) wurde nie angezeigt
       (Z.162 sprang immer ins iframe) und ist entfernt. Logik (qcCalc/qcSaveAsObject/
       qcImportPdfTrigger etc.) liegt ausserhalb dieser Funktion und bleibt unveraendert. */
    var host = document.getElementById('qc-tab-host');
    if (host && window.__qcMountIframe) { window.__qcMountIframe(host); return; }
    /* Fallback (sollte praktisch nie passieren: Bruecke fehlt) — nur Hinweis anzeigen. */
    if (host && host.dataset.rendered !== '1') {
      host.innerHTML = '<p class="hint" style="text-align:center;padding:60px 20px;color:var(--muted)">Quick-Check konnte nicht geladen werden (Bridge fehlt). Bitte Seite neu laden.</p>';
