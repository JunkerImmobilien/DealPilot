/* W31-wl-token: Whitelabel-Farbe zur Laufzeit aufloesen.
   Canvas und SVG-Praesentationsattribute verstehen KEIN var() — dort geht es nur so.
   Ohne Whitelabel liefert _wlc() das Literal zurueck -> Optik unveraendert.
...
(function (global) {
  ...
  function lageCol(s) { return s == null ? '#bdb6a6' : s >= 55 ? '#3FA56C' : s >= 40 ? 'var(--wl-c9a84c, #C9A84C)' : '#B8625C'; }
  ...
  function uDealpilot(o) {
    ...
    var accCol = pct == null ? '#7A7370' : (pct >= 75 ? '#3FA56C' : pct >= 50 ? 'var(--wl-9a7f33, #9a7f33)' : '#B8625C');
    function band(k) { return D.mw ? D.mw[k] : null; }
    function bandM(k) { return D.mm ? D.mm[k] : null; }
    return {
      key: 'dp', name: 'DealPilot', isDP: true, brand: 'var(--wl-c9a84c, #C9A84C)',
  ...
  function injectCss() {
    ...
    var P = '#avsec ';
    var css = [
      '#avsec{margin:0 0 14px;border:1px solid color-mix(in srgb, var(--wl-c9a84c, #C9A84C) 32%, transparent);border-radius:14px;overflow:hidden;background:#ffffff;box-shadow:0 12px 32px -22px rgba(120,90,20,.55)}',
      P + '*{box-sizing:border-box}',
      P + '.av-bar{display:flex;align-items:center;gap:12px;flex-wrap:wrap;padding:10px 16px;background:#ffffff;border-bottom:1px solid color-mix(in srgb, var(--wl-c9a84c, #C9A84C) 32%, transparent)}',
      ...
      P + '.av-kons .kt{font:700 8.5px/1 "JetBrains Mono",monospace;letter-spacing:1.4px;text-transform:uppercase;color:var(--wl-b8932f, #b8932f)}',
      ...
      P + '.avs-kpill{font:700 10.5px/1 "JetBrains Mono",monospace;color:#6b5d3a;background:color-mix(in srgb, var(--wl-c9a84c, #C9A84C) 15%, transparent);border:1px solid color-mix(in srgb, var(--wl-c9a84c, #C9A84C) 32%, transparent);border-radius:999px;padding:5px 11px;white-space:nowrap}',
      ...
      P + '.av-seg{display:inline-flex;border:1px solid var(--wl-d8c79a, #d8c79a);border-radius:8px;overflow:hidden;background:#fff}',
      ...
      P + '.av-seg button+button{border-left:1px solid var(--wl-ece0c2, #ece0c2)}',
      P + '.av-seg button.avs-sel{background:#fff;color:var(--wl-b8932f, #b8932f);box-shadow:inset 0 0 0 1.5px var(--wl-c9a84c, #C9A84C)}',
      P + '.av-vsw{display:inline-flex;gap:5px}',
      P + '.av-vsw button{font:700 11px/1 "JetBrains Mono",monospace;color:#8a7f63;background:#fff;border:1px solid var(--wl-d8c79a, #d8c79a);border-radius:8px;padding:8px 11px;cursor:pointer;display:flex;align-items:center;gap:5px}',
      P + '.av-vsw button svg{width:13px;height:13px}',
      P + '.av-vsw button.avs-on{color:var(--wl-b8932f, #b8932f);background:#fff;border-color:var(--wl-c9a84c, #C9A84C);box-shadow:inset 0 0 0 1px var(--wl-c9a84c, #C9A84C)}',
      P + '.av-min{font:700 11px/1 "JetBrains Mono",monospace;color:#8a7f63;cursor:pointer;background:#fff;border:1px solid var(--wl-d8c79a, #d8c79a);border-radius:8px;padding:8px 11px}',
      P + '.av-btn{position:relative;overflow:hidden;display:inline-flex;align-items:center;justify-content:center;gap:6px;font:700 11.5px/1 "JetBrains Mono",monospace;border:1px solid var(--wl-c9a84c, #C9A84C);border-radius:9px;padding:8px 13px;cursor:pointer;background:#fff;color:#1b1815;box-shadow:none}',
      P + '.av-btn.sm{padding:8px 13px;font-size:11px}',
      P + '.av-btn.full{width:100%}',
      P + '.av-ghost{display:inline-flex;align-items:center;justify-content:center;gap:6px;font:700 10.5px/1 "JetBrains Mono",monospace;border:1px solid var(--wl-d8c79a, #d8c79a);border-radius:9px;padding:8px 12px;cursor:pointer;background:#fff;color:#1b1815;width:100%}',
      ...
      '#oab-credit-hint.avs-credit-below{margin:13px 0 14px !important;padding:9px 16px !important;background:linear-gradient(120deg,var(--wl-fbf3df, #fbf3df),var(--wl-f6ead0, #f6ead0)) !important;border:1px solid color-mix(in srgb, var(--wl-c9a84c, #C9A84C) 32%, transparent) !important;border-radius:12px !important;font:12.5px/1.45 "JetBrains Mono",monospace !important;color:#5f5236 !important}',
      ...
      /*v877-no-hatch*/P + '.mhead.dphead{position:relative;overflow:hidden;background:linear-gradient(110deg,var(--wl-e8cc7a, #E8CC7A),var(--wl-c9a84c, #C9A84C) 58%,var(--wl-b8932f, #b8932f))}',
      ...
      P + '.mhead.dphead .avs-cf{color:var(--wl-e8cc7a, #E8CC7A);background:#0c0b09}',
      P + '.mperf{position:relative;height:0;border-top:1.5px dashed color-mix(in srgb, var(--wl-c9a84c, #C9A84C) 50%, transparent);margin:0 12px}',
      ...
      P + '.mbody .mw.dpval{background:linear-gradient(110deg,var(--wl-b8932f, #b8932f),var(--wl-c9a84c, #C9A84C) 55%,var(--wl-9a7f33, #9a7f33));-webkit-background-clip:text;background-clip:text;color:transparent}',
      ...
      P + '.f3{flex:1;border:1px solid var(--wl-efe7d2, #efe7d2);border-radius:9px;padding:6px 8px;min-width:0;background:#ffffff}',
      ...
      P + '.mx th{font:700 8.5px/1 "JetBrains Mono",monospace;letter-spacing:.7px;text-transform:uppercase;color:#6b6660;text-align:right;padding:8px 9px;border-bottom:1px solid color-mix(in srgb, var(--wl-c9a84c, #C9A84C) 22%, transparent)}',
      ...
      P + '.mx .cons{background:#fcfaf4;color:var(--wl-b8932f, #b8932f)}',
      P + '.mx .pos{color:#2f8f57}.mx .neg{color:#c0564f}.mx .na{color:#bdb6a6}',
      P + '.mx .pill{display:inline-flex;gap:5px;align-items:center}',
      P + '.mx .pill .dpm{width:13px;height:13px;border-radius:4px;background:#0c0b09;display:inline-flex;align-items:center;justify-content:center;font-size:7px;color:var(--wl-e8cc7a, #E8CC7A)}',
  ...
  function accUnified(u) {
    var map = { hoch: ['Hoch', '#3FA56C'], mittel: ['Mittel', 'var(--wl-9a7f33, #9a7f33)'], niedrig: ['Niedrig', '#B8625C'] };
  ...
  function topbar(ps) {
    ...
    if (ps.length >= 2) {
      ...
      k = '<div class="av-kons">' +
        '<span class="avs-kicon"><svg viewBox="0 0 24 24" fill="none" stroke="' + window._wlc('#E8CC7A') + '" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"/><path d="m7 14 4-4 4 4 5-5"/></svg></span>' +
