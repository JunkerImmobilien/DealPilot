/* ============================================================================
   DealPilot v852 – admin-network.js  (NEU, additiv, laedt NACH admin-app.js)
   Karten-Designer fuer Netzwerk-Partnerkarten:
     Optik (Hintergrund 6 Presets + freie Farbe, Abrisskante 4 Stile + Farbe,
     Akzent, Logo, Kuerzel), Inhalt (Name/Rolle/Tags/Beschreibung/USP/Antwortzeit/
     Verified/CTA), Mitgabe-Checkboxen, Anforderungs-Checkboxen (Anfrage-Sperre),
     Live-Boarding-Pass-Vorschau, Liste mit Leads-Zaehler.
   Erzeugt Nav-Link "Netzwerk" + #view-network SELBST.
   CRUD ueber /api/v1/admin/network-cards.
   ============================================================================ */
(function () {
  'use strict';
  ...
  function _injectCss() {
    ...
    var css = [
      '#view-network .adn-wrap{display:grid;grid-template-columns:1fr 440px;gap:20px;align-items:start}',
      '@media(max-width:1100px){#view-network .adn-wrap{grid-template-columns:1fr}}',
      ...
      '#view-network .adn-litem{display:flex;align-items:center;gap:12px;padding:11px 12px;border:1px solid #eee;border-radius:9px;margin-bottom:8px;background:#fff;flex-wrap:wrap}',
      ...
      /* Vorschau-Karte (Spiegel der App-Styles) */
      '#view-network .pv{position:relative;width:100%;max-width:420px;display:flex;background:var(--bpbg,#fff);color:#2A2727;border:1px solid rgba(201,168,76,.24);border-radius:15px;overflow:hidden;box-shadow:0 4px 16px rgba(42,39,39,.1)}',
      ...
      '#view-network .pv-logo{width:46px;height:46px;border-radius:11px;flex-shrink:0;overflow:hidden;border:1px solid rgba(42,39,39,.1);display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-family:sans-serif;font-size:16px}',
      '#view-network .pv-logo img{width:100%;height:100%;object-fit:cover}',
  ...
  function _formData() {
    ...
    return {
      ...
      sortierung: parseInt(_val('adn-sort'), 10) || 0
    };
  }
  ...
  function _renderPreview() {
    ...
    var logo = d.logo_url ? '<img src="' + _esc(d.logo_url) + '" alt="">'
    ...
    var bgCls = '', bgStyle = '';
    if (d.hintergrund === 'custom' && d.hintergrund_farbe) bgStyle = 'background:' + _esc(d.hintergrund_farbe) + ';';
    ...
    host.innerHTML =
      '<div class="pv' + bgCls + '" style="--acc:' + _esc(acc) + ';' + bgStyle + '">' +
        ...
        meta + req +
        '<button class="pv-cta" type="button"><span class="t">' + _esc(d.cta_label || 'Anfrage senden') + '</span><span class="s">kostenlos &amp; unverbindlich</span></button>' +
        '</div>' +
  ...
  function _renderList() {
    ...
    host.innerHTML = _cards.map(function (c) {
      ...
      return '<div class="adn-litem">' +
        '<span class="adn-badge">' + _esc(c.kategorie) + '</span>' +
  ...
  function _fill(c) {
    ...
    set('adn-kategorie', c ? c.kategorie : 'finanzierung');
  ...
  function _syncVisibility() {
    ...
    if (bgc) bgc.style.display = (_val('adn-bg') === 'custom') ? '' : 'none';
  ...
  function _render(host) {
    host.innerHTML =
      ...
      '<div class="adn-wrap">' +
        '<div class="adn-card">' +
          ...
          '<div class="adn-row2">' +
            '<div class="adn-field"><label>Kategorie</label><select id="adn-kategorie"><option value="finanzierung">Finanzierung &amp; Banken</option><option value="gutachter">Gutachter &amp; Sachverst\u00e4ndige</option></select></div>' +
          ...
          '<div class="adn-field"><label>Tags (Komma-getrennt, max. 4 sichtbar)</label><input id="adn-tags" placeholder="450 Banken, ungebunden, bundesweit"></div>' +
          ...
          '</div>' +
          '<div class="adn-row2">' +
            '<div class="adn-field"><label>K\u00fcrzel (Abriss)</label><input id="adn-kuerzel" maxlength="4" placeholder="SPR"></div>' +
            '<div class="adn-field"><label>Logo-URL (optional)</label><input id="adn-logo" placeholder="https://\u2026/logo.png"></div>' +
          '</div>' +
    ...
    var ids = ['adn-kategorie', 'adn-name', 'adn-rolle', 'adn-tags', 'adn-beschreibung', 'adn-usp',
      'adn-antwortzeit', 'adn-verified', 'adn-cta', 'adn-akzent', 'adn-bg', 'adn-bgfarbe',
      'adn-kante', 'adn-kantefarbe', 'adn-kante-auto', 'adn-kuerzel', 'adn-logo'];
